...
const MusicPlayer = () => {
    ...
    useEffect(() => {
        if (song?.audioURL) {
            ...
            const updateTime = () => {
                ...
                setProgress((current / (audio.duration || 1)) * 100);
            };
    ...
    if (!song) {
        return <>
            <div className="fixed bottom-0 z-50 grid grid-cols-3 items-center w-full h-20 border-t border-gray-700 bg-[#121212] px-4 ">
                <div className="flex items-center justify-between gap-4 h-full p-3 transition-colors rounded-lg">
                    <div className="flex items-center flex-col sm:flex-row sm:justify-between sm:items-center space-x-4">
                        <img loading="lazy" src='cat.png' alt='img' className="w-12 h-12 rounded-md object-cover"/>
                        <div>
                ...
                <div className="h-full flex flex-col items-center p-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div
                            className="relative w-80 h-1 bg-gray-600 rounded overflow-hidden cursor-pointer">
                            <div className="absolute top-0 left-0 h-full bg-white" style={{ width: '80%' }}></div>
                        </div>
                        <span className="text-white text-xs">00:00</span>
                    </div>
                </div>
                ...
                <div className="flex h-full items-center justify-end gap-4 pr-4">
                    <button onClick={() => setIsVolumeMute(!isVolumeMute)} className="transition-transform duration-200 hover:scale-110">
                        {isVolumeMute ? '🔇' : '🔊'}
                    </button>
    ...
    return (
        <>
            <div className='border border-white w-full h-screen'></div>
            <div className="fixed bottom-0  z-50 grid grid-cols-3 items-center w-full h-20 bg-[#0d0d0e3f]">
                {/* Left: Song Info */}
                <div
                    key={song.id}
                    className="flex items-center justify-between gap-4 h-full p-3 transition-colors rounded-lg"
                    onClick={() => console.log('Selected:', song)}
                >
                    <div className="flex items-center flex-col sm:flex-row sm:justify-between sm:items-center space-x-4">
                        <img
                            loading="lazy"
                    ...
                    >
                        {isLiked ?
                            ...
                            </svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                ...
                <div className="h-full flex flex-col items-center p-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div
                            className="h-1 border border-white w-80 hover:cursor-pointer bg-black relative"
                            onClick={handleSeek}
                        >
                            <div
                                className="absolute top-[-4px] left-0 h-full bg-white p-1 rounded-full border border-white"
                ...
                {/* Right: Volume Controls */}
                <div className="flex h-full items-center justify-start gap-4 pr-4">
                    <button onClick={() => setIsVolumeMute(!isVolumeMute)} className="mt-1">
                        {isVolumeMute ?
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" color="#ffffff" fill="white">
                    ...
                    <div className="h-1 border border-white w-16 bg-black relative">
